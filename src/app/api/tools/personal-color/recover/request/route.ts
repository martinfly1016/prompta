...
// POST { email } → if credits exist for that email, send a recovery link.
// Always returns 200 with the same neutral message to avoid leaking which
...
export async function POST(req: NextRequest) {
  ...
  // Always respond positively — do NOT reveal whether the email has credits.
  // If account exists, send the email; otherwise silently no-op.
  if (account && account.balance > 0) {
    const token = signRecoveryToken(eh)
    const link = `${SITE_CONFIG.url}/tools/personal-color-analysis?recover=${encodeURIComponent(token)}`
    const subject = '【prompta.jp】パーソナルカラー診断クレジットの復元リンク'
    const text = `prompta.jp パーソナルカラー診断ツールをご利用いただきありがとうございます。
...
`
  ...
  <p>prompta.jp パーソナルカラー診断ツールをご利用いただきありがとうございます。</p>
  ...
  return NextResponse.json({ ok: true, message: '指定のメールアドレスにクレジットが紐づいている場合、復元リンクを送信しました。' })
}
